...
def add_transaction():
    ...
    if not values:
        ...
        return jsonify(response), 400
    required_field = ['recipient', 'amount','memo']
    ...
    amount = values['amount']
    memo = values['memo']
    signature = wallet.sign_transaction(wallet.public_key, recipient, amount,memo)
    success = blockchain.add_transaction(
        recipient, wallet.public_key, signature, amount,memo)
    if success:
        response = {
            'message': 'Successfully added transaction.',
            'transaction': {
